...
const baseUrl = (process.env.PRODUCTION_URL || 'https://cyhaj.app').replace(/\/$/, '')
const supabaseUrl = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL
const anonKey = process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY
const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SECRET_KEY
const pythonExe = process.env.PRODUCTION_VERIFY_PYTHON
if (![supabaseUrl, anonKey, serviceKey, pythonExe].every(Boolean)) throw new Error('Production verification environment is incomplete')
...
const localPredictions = payloads.map((payload) => {
  const run = spawnSync(pythonExe, ['-c', pythonCode, JSON.stringify(payload)], {
    cwd: process.cwd(), encoding: 'utf8', env: { ...process.env, PYTHONPATH: path.resolve('.python-deps') }, maxBuffer: 1024 * 1024,
  })
...
const watchdog = setTimeout(() => {
  console.error('[production-verify] timed out')
  process.exit(2)
}, 240000)
...
async function completeBoot() {
  if (await page.locator('#app .vault-console, #app .survivor-auth-shell').count()) return
...
await page.goto(`${baseUrl}/?verify=production#/start`, { waitUntil: 'domcontentloaded', timeout: 120000 })
await completeBoot()
console.error('[production-verify] boot complete')
...
for (const [name, response] of [['health', remote.health], ['options', remote.options], ...remote.predictions.map((item, index) => [`prediction${index + 1}`, item]), ['repeat', remote.repeatedPrediction]]) {
  if (response.status !== 200 || !response.contentType.includes('application/json')) throw new Error(`${name} contract failed`)
}
if (remote.options.body.countries.length < 200 || remote.options.body.disaster_types.length < 2) throw new Error('Production options are incomplete')
if (JSON.stringify(remote.health.body.classes) !== JSON.stringify(['Low', 'Moderate', 'Severe'])) throw new Error('Production class order changed')
let maxProbabilityDifference = 0
for (const [index, prediction] of remote.predictions.entries()) {
  if (prediction.body.prediction.level !== localPredictions[index].body.prediction.level) throw new Error('Production label differs from local frozen pipeline')
...
if (maxProbabilityDifference > 1e-12) throw new Error('Production probabilities differ from local frozen pipeline')
if (JSON.stringify(remote.predictions[0].body) !== JSON.stringify(remote.repeatedPrediction.body)) throw new Error('Repeated production prediction changed')
console.error('[production-verify] api consistency complete')

const projectRef = new URL(supabaseUrl).hostname.split('.')[0]
const storageKey = `sb-${projectRef}-auth-token`
const installSession = (session) => page.evaluate(({ key, value }) => localStorage.setItem(key, JSON.stringify(value)), { key: storageKey, value: session })
const routeChecks = []
async function verifyRoute(hash, expected = hash) {
  console.error(`[production-verify] route ${hash}`)
  await page.goto(`${baseUrl}/?route-check=${encodeURIComponent(hash)}#/${hash}`, { waitUntil: 'domcontentloaded', timeout: 120000 })
  await completeBoot()
  await page.waitForFunction((value) => location.hash === `#/${value}`, expected, { timeout: 30000 })
  const textLength = await page.locator('#app').innerText().then((text) => text.trim().length)
  if (!textLength) throw new Error(`Empty route: ${hash}`)
  routeChecks.push(hash)
}

await page.evaluate((key) => { localStorage.removeItem(key); sessionStorage.removeItem('crisis_user') }, storageKey)
await page.goto(`${baseUrl}/?verify=unauth#/admin/dashboard`, { waitUntil: 'domcontentloaded', timeout: 120000 })
await completeBoot()
await page.waitForFunction(() => location.hash === '#/auth', null, { timeout: 30000 })
const unauthRoute = await page.evaluate(() => location.hash)
if (!(await page.getByRole('tab', { name: '登录' }).count()) || !(await page.getByRole('tab', { name: '注册' }).count())) throw new Error('Auth page tabs unavailable')
console.error('[production-verify] unauth guard complete')

const userAuth = await sessionFor(USER_ID)
await installSession(userAuth.session)
await page.goto(`${baseUrl}/?verify=ordinary#/admin/dashboard`, { waitUntil: 'domcontentloaded', timeout: 120000 })
await completeBoot()
await page.waitForFunction(() => location.hash === '#/user/overview', null, { timeout: 30000 })
const ordinaryRoute = await page.evaluate(() => location.hash)
console.error('[production-verify] ordinary guard complete')
...
await installSession(refreshed.session)
await verifyRoute('user/overview')
...
await page.goto(`${baseUrl}/?verify=forged#/admin/dashboard`, { waitUntil: 'domcontentloaded', timeout: 120000 })
await completeBoot()
await page.waitForFunction(() => location.hash === '#/auth', null, { timeout: 30000 })
const forgedRoute = await page.evaluate(() => location.hash)
console.error('[production-verify] forged guard complete')

const adminAuth = await sessionFor(ADMIN_ID)
await page.evaluate((key) => { sessionStorage.removeItem('crisis_user'); localStorage.removeItem(key) }, storageKey)
await installSession(adminAuth.session)
for (const route of ['admin/dashboard', 'admin/disasters', 'admin/users', 'admin/system', 'admin/models']) await verifyRoute(route)
console.error('[production-verify] admin routes complete')
...
await page.goto(`${baseUrl}/?verify=after-signout#/admin/dashboard`, { waitUntil: 'domcontentloaded', timeout: 120000 })
await completeBoot()
await page.waitForFunction(() => location.hash === '#/auth', null, { timeout: 30000 })
const afterSignoutRoute = await page.evaluate(() => location.hash)

const authSource = readFileSync('src/auth.js', 'utf8')
const signUpBlock = authSource.slice(authSource.indexOf('export async function signUp'), authSource.indexOf('export async function signOut'))
if (!signUpBlock.includes('supabase.auth.signUp') || /app_metadata|role\s*:/.test(signUpBlock)) throw new Error('Registration role boundary changed')
...
console.log(JSON.stringify({
  ...
  auth: { unauth_route: unauthRoute, ordinary_admin_route: ordinaryRoute, forged_route: forgedRoute, refresh: true, logout_route: logoutRoute, after_signout_route: afterSignoutRoute, registration_role_boundary_static: true },
  routes: routeChecks,
